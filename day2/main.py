input = """Game 1: 7 blue, 6 green, 3 red; 3 red, 5 green, 1 blue; 1 red, 5 green, 8 blue; 3 red, 1 green, 5 blue
...
def day1():
    ...
    def check_is_valid(game_string):
        cube_set_array = game_string.split('; ')
        for cube_set_string in cube_set_array:
            cube_string_array = cube_set_string.split(', ')
            for cube_string in cube_string_array:
                [value, cube] = cube_string.split(' ')
                if int(value) > max_index[cube]:
                    return False    
        return True
    total = 0
    for i, line in enumerate(lines):
        [_, game_string] = line.split(': ')
        if check_is_valid(game_string):
            total += i + 1
    return total
...
def day2():
    lines = input.split('\n')
    def get_power_of_min_set(game_string):
        max_index = {"red": 0, "green": 0, "blue": 0}
        cube_set_array = game_string.split('; ')
        for cube_set_string in cube_set_array:
            cube_string_array = cube_set_string.split(', ')
            for cube_string in cube_string_array:
                [value, cube] = cube_string.split(' ')
                if int(value) > max_index[cube]:
                    max_index[cube] = int(value)
        power = 1
        for value in max_index.values():
            power *= value
        return power
    total = 0
    for i, line in enumerate(lines):
        [_, game_string] = line.split(': ')
        total += get_power_of_min_set(game_string)
    return total
