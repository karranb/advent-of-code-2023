...
def part_1():
    total = 0
    for line in lines:
        parsed_line = re.findall('\d', line)
        total += int(parsed_line[0]+parsed_line[-1])
    return total
...
def part_2():
    ...
    for line in lines:
        ...
        for char_index in range(len(line)):
            ...
            for key, value in translation_index.items():
                if line[char_index:].startswith(key):
                    parsed_line += [value]
        char_1 = parsed_line[0]
        char_2 = parsed_line[-1]
        total += int(char_1+char_2)
